	...
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		if (bStatusValid)
		{
			...
			if (!SlotIsEmpty(Slot))	//there is an ability in this slot already, deactivate and clear its slot.
			{
				...
				if (SpecWithSlot)
				{
					if (AbilityTag.MatchesTagExact(GetAbilityTagFromSpec(*SpecWithSlot)))	//is that ability same  as this, so we can return early
					{
			...
			if (!AbilityHasAnySlot(*AbilitySpec)) //ability doesn't have a slot yet
			{
