...
#include "AbilitySystem/ExecCalc/ExcCalc_Damage.h"
...
UExcCalc_Damage::UExcCalc_Damage()
{
...
void UExcCalc_Damage::DetermineDebuff(const FGameplayEffectCustomExecutionParameters& ExecutionParams, const FGameplayEffectSpec& Spec, FAggregatorEvaluateParameters EvaluationParameters,
	const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -.5f) // .5 padding for floating point [im]precision
		{
			...
			if (bDebuff)
			{
				//TODO do somthn about this!?!?!
			}
		}
	}
}

void UExcCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams,
                                             FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	// Capture block chance from target, and determine if there was a successful block
	...
	//Critical Hit Resistance reduces crit hit chance by percent and doubling if the crit chance ok
	...
	Damage = bCriticalHit ? Damage = 2.f * Damage + SourceCriticalHitDamage : Damage;
